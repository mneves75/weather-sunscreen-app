 ...
 *
 * Uses utility functions to get i18n keys, then translates with react-i18next.
 * See: src/utils/weather.ts, src/utils/units.ts for key generation
 * See: src/types/i18n.ts for type-safe key definitions
 */
...
export const WeatherDetails = React.memo<WeatherDetailsProps>(({ 
  ...
  const windDesc = getWindDescription(data.current.windSpeed);  // e.g., 'weather.windLevels.calm'
  const humidityLevel = getHumidityLevel(data.current.humidity);  // e.g., 'weather.humidityLevels.dry'
