...
import { SunscreenApplication, UserSunscreenProfile } from '../types/sunscreen';
...
export function SunscreenProvider({ children }: SunscreenProviderProps) {
  ...
  const logApplication = useCallback(
    async (spf: number, bodyParts: string[], notes?: string) => {
      try {
  ...
  return <SunscreenContext.Provider value={contextValue}>{children}</SunscreenContext.Provider>;
}
