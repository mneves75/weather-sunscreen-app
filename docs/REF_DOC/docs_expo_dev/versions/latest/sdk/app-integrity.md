...
`@expo/app-integrity` uses Play Integrity's [Standard request
...
A Promise that is fulfilled with a string that contains the attestation data.
...
### `AppIntegrity.requestIntegrityCheck(requestHash)`
